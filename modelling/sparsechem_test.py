...
for model_seed in config_options['seed']: 
    ...
    best_test_pred_per_task_per_fold = []
    for val_i in range(1,5):
        ...
        best_va_r2_per_task, best_test_pred_per_task = None, None
        for epoch in range(conf.epochs):
            ...
            # compute and save TEST predictions if VALIDATION is best in current epoch
    ...
    exp.test_metric_df = own_compute_metrics_repr(exp.Y_test, ensemble_best_test_pred_per_task, col_names=conf.measurement_cols)
    exp.test_metric_df = exp.test_metric_df[exp.test_metric_df['target']==test_col]
    ...
    if conf.measurement_cols != train_measurement_cols:
        param_dict["orig_measurement_cols"] = train_measurement_cols
        
    print(param_dict)
    
    exp.save_results(parameters=param_dict, save_fold_results=True, database_name=args.db_name, table_name=args.db_table_name)  
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))
